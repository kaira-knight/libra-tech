...
import { Typography, List, ListItem, Divider, Button, Box, Avatar, TextField, Dialog, DialogActions, DialogContent, DialogTitle, Snackbar, Alert, Card, CardContent } from '@mui/material';
...
const Profile = () => {
  const [user, setUser] = useState({
    ...
    membership: "Premium",
    username: "team_realistic",  // New username field
    avatar: "https://upload.wikimedia.org/wikipedia/commons/2/20/SITBihar.jpg", // Default Avatar URL
    borrowedBooks: [
  ...
  // Animation for smooth entry of profile elements
  const profileFadeIn = useSpring({
    opacity: 1,
    from: { opacity: 0 },
    config: { duration: 500 },
  });

  // Animation for borrowed books list items
  const bookListItemSpring = useSpring({
    opacity: 1,
  ...
  const handleUpgradeMembership = () => {
    setUser((prevState) => ({
      ...prevState,
      membership: prevState.membership === "Premium" ? "Elite" : "Premium",
    }));
    setNotification({
      open: true,
    ...
    <div style={{ backgroundColor: '#f9f9f9', padding: '20px' }}>
      <animated.div style={profileFadeIn}>
        ...
        <Card sx={{ maxWidth: 600, margin: '0 auto', padding: '20px', boxShadow: 3 }}>
          <CardContent>
            <Box display="flex" alignItems="center" flexDirection="column" mb={3} justifyContent="center">
        ...
        {/* Borrowed Books Section with Animation */}
        <Box mt={8}>
